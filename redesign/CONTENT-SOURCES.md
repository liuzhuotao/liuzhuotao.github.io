...
The site displays 94 research items, with three
...
| Site content | Repository source | Treatment |
| --- | --- | --- |
...
prose biography are the sources for those subjects. The original site's resume
link pointed to unverified template content, so it was not carried over.

The latest source news is August 2026. No September 2026 announcement is invented.
The selected-paper choices remain editable in their individual entries.
...
For launch, the About text is the full **Academic Profile** paragraph from
`Zhuotao_Liu_CV_Final.docx`, used verbatim at the user's explicit request. This
supersedes the earlier shortened first-person biography. Selection remains
controlled by each paper's `selected` field, without a fixed number of papers.
Production builds use the `redesign/` source, allow indexing, and generate a
sitemap. The `preview` environment retains the notice and indexing block.

Legacy navigation URLs for news, awards, the author profile, experience, and
research topics redirect to the homepage. Old publication-category URLs redirect
to the complete publication list. The original Jinzhu Yan CV URL is preserved
with a verbatim copy of the already-public PDF.
